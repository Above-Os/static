...
restore_k8s_os() {
    ...
    ensure_success $sh_c "${VELERO} -n os-system restore create --status-include-resources $include_status_rgs --status-exclude-resources distributedredisclusters.redis.kun,perconaservermongodbs.psmdb.percona.com --selector 'managered-by notin (mongo-backup-mongo-cluster,mongo-restore-mongo-cluster)' --from-backup $backupName"
